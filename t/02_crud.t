...
my $concert
    = $artist->concerts->create( { location => 'Madison Cube Garden', } );

lives_ok(
    sub {
        $artist->albums->create( { id => 1, name => 'Mach et einfach!' } );
    },
    'create album'
);
...
# test / for no special reason
{
    ...
    like( $res->decoded_content, '/TestApp/',
...
# SHOW
{
    ...
    my $res  = request($path);
    ok( $res->is_success, "$path returns HTTP 200" );
    like( $res->decoded_content, '/davewood/',
        "$path content contains string 'davewood'" );
    $path = '/artists/99/show';
    ok( request($path)->code == 404,
        "Unknown resource $path returns HTTP 404" );
}
...
# LIST
{
    ...
    like( $res->decoded_content, '/davewood[\s\S]*flipper/',
        "$path content contains 'davewood' and 'flipper'" );
}
...
# CREATE
{
    ...
    my $res  = request($path);
    ok( $res->is_success, "$path returns HTTP 200" );
    like( $res->decoded_content, '/method="post".*password/s',
        "$path content contains 'method=\"post\"'" );
    $res = request( POST $path,
        [ name => 'simit', password => 'asdf', password_repeat => 'asdf' ] );
    ok( $res->is_redirect, "$path returns HTTP 302" );
    $path = '/artists/list';
    $res  = request($path);
    like( $res->decoded_content, '/simit/',
        "$path content contains 'simit'" );
}
...
# EDIT
{
    ...
    $path = '/artists/2/show';
    $res  = request($path);
    like( $res->decoded_content, '/willy/',
        "$path content contains 'willy'" );
}
...
# SHOW
{
    ...
    like(
        $res->decoded_content,
        '/Mach et einfach/',
        "$path content contains string 'Mach et einfach'"
    );
...
# LIST
{
    ...
    like(
        $res->decoded_content,
        '/Mach et einfach/',
        "$path content contains 'Mach et einfach'"
    );
}
...
# CREATE
{
    ...
    my $res  = request($path);
    ok( $res->is_success, "$path returns HTTP 200" );
    like( $res->decoded_content, '/method="post"/',
        "$path content contains 'method=\"post\"'" );
    ...
    like(
        $res->decoded_content,
        '/I Brake Together/',
        "$path content contains 'I Brake Together'"
    );
}
...
# EDIT
{
    ...
    like(
        $res->decoded_content,
        '/Es gibt Reis, Baby/',
        "$path content contains 'Es gibt Reis, Baby'"
    );
}
